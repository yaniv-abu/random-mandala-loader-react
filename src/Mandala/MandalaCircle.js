...
export default function MandalaCircle ({numOfShapesInCurrentCircle, circleIndex, shapeBounds, style={}}) {
    const shapesArray = [];
    ...
    for (let i = 1; i <= numOfShapesInCurrentCircle; i++) {
        ...
        const rotationStyle = {transform: 'rotateZ(' + currentShapeRotation + 'deg)'};
        const shapeClone = <MandalaBaseShape key={i} style={rotationStyle} shapePathData={shapePathData} />;
        shapesArray.push(shapeClone);
    }
    return (<g key={circleIndex} style={style}>{shapesArray}</g>);
};
